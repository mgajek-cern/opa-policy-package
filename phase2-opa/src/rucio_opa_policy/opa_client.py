...
"""
Thin synchronous client for the OPA REST API.

Kept deliberately minimal: one method that posts an input document
to a named policy rule and returns the boolean result.

Configuration is read from environment variables so the package works
in containerised deployments without touching Rucio config files:

    OPA_URL          — base URL of the OPA server  (default: http://localhost:8181)
    OPA_POLICY_PATH  — Rego rule path to query      (default: vo/authz/allow)
...
_DEFAULT_POLICY_PATH = "vo/authz/allow"
_DEFAULT_TIMEOUT = 2
...
def query_opa(input_doc: dict[str, Any]) -> bool:
    ...
    False for ``false`` *and* on any connection/timeout error
    (fail-closed behaviour).

    Args:
        input_doc: The ``input`` document that Rego policies receive.
    """
    url = _opa_url()
