...
from fts3rest.model.meta import Session, init_model
    ...
    with open(config_file) as config:
        ...
        for line in config:
            if not line.isspace() and not line.lstrip().startswith("#"):
                ...
                break
        if not content:
            raise OSError("Empty configuration file")
...
def create_app(default_config_file=None, test=False):
    ...
    if fts3cfg["fts3.DbType"] == "postgresql" and not fts3cfg["fts3.ExperimentalPostgresSupport"]:
        raise ValueError(
    ...
    def handle_exception(e):
        ...
        response.data = json.dumps({"status": f"{e.code} {e.name}", "message": e.description})
