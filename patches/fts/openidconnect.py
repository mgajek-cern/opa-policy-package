...
class OIDCmanager:
        ...
        for provider in providers_config:
            try:
                ...
                if "introspection_endpoint" not in client.provider_info:
                    log.info(f"{issuer} -- missing introspection endpoint")
                self.clients[issuer] = client
            except Exception as ex:
                log.warning(f"Exception registering provider: {provider}")
                log.warning(ex)
    ...
    def token_issuer_supported(self, access_token):
        ...
        log.debug(f"Checking client registration for issuer={issuer}")
        log.debug(f"Supported issuers={list(self.clients.keys())}")
    ...
    def filter_provider_keys(self, issuer, kid=None, alg=None):
        ...
        if client is None:
            raise ValueError(f"Could not retrieve client for issuer={issuer}")
