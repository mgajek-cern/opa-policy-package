...
from rucio.common.constants import (
    DEFAULT_VO,
    RSE_ALL_SUPPORTED_PROTOCOL_OPERATIONS,
    RSE_ATTRS_BOOL,
    RSE_ATTRS_STR,
...
# WLCG scope intent → EGI Check-In vocabulary
_EGI_SCOPE_MAP = {
    "storage.read": "read:/",
    "storage.modify": "write:/",
...
class RseData:
    ...
    def is_tape(self) -> bool:
        if (
            self.info["rse_type"] == RSEType.TAPE
            or self.info["rse_type"] == "TAPE"
            or self.attributes.get(RseAttr.STAGING_REQUIRED, False)
        ):
            return True
        return False
    ...
    def bulk_load(
        ...
        *,
        ...
        """
        if (
            len(rse_id_to_data) < 4
        ):  # 4 was selected without particular reason as "seems good enough"
        ...
        stmt = select(models.RSE).join_from(temp_table, models.RSE, models.RSE.id == temp_table.id)
        ...
        if load_attributes:
            for rse_id, attr in _fetch_many_rses_attributes(
                rse_id_temp_table=temp_table, session=session
            ):
        ...
        if load_columns:
            ...
            if not load_attributes:
                settings_by_id = dict(
                    _fetch_many_rses_attributes(
                        rse_id_temp_table=temp_table, keys=RSE_SETTINGS, session=session
                    )
                )
            for rse_id, db_rse in db_rses_by_id.items():
                rse_data = rse_id_to_data[rse_id]
                settings = (
                    rse_data._attributes
                    if rse_data._attributes is not None
                    else settings_by_id.get(rse_id, {})
                )
        ...
        if load_info:
            stmt = (
                select(temp_table.id, models.RSEProtocol)
                .outerjoin_from(
                    temp_table,
                    models.RSEProtocol,
            ...
            for rse_id, db_protocols in _group_query_result_by_rse_id(stmt, session=session):
                ...
                info = _format_get_rse_protocols(
                    rse=db_rse,
                    db_protocols=db_protocols,
        ...
        if load_limits:
            stmt = (
                select(temp_table.id, models.RSELimit)
                .outerjoin_from(
                    temp_table, models.RSELimit, models.RSELimit.rse_id == temp_table.id
                )
                .order_by(
                    temp_table.id,
                )
        ...
        if load_usage:
            stmt = (
                select(temp_table.id, models.RSEUsage)
                .outerjoin_from(
                    temp_table, models.RSEUsage, models.RSEUsage.rse_id == temp_table.id
                )
                .order_by(
                    temp_table.id,
                )
...
class RseCollection(Generic[T]):
    ...
    def ensure_loaded(
        ...
        *,
        ...
        RseData.bulk_load(
            rse_id_to_data={rse_id: self.get_or_create(rse_id) for rse_id in rse_ids}
            if rse_ids
            else self.rse_id_to_data_map,
            load_name=load_name,
            load_columns=load_columns,
            load_attributes=load_attributes,
            load_info=load_info,
...
def _group_query_result_by_rse_id(
    stmt: "Executable", *, session: "Session"
...
@read_session
def rse_exists(
    rse: str, vo: str = DEFAULT_VO, include_deleted: bool = False, *, session: "Session"
) -> bool:
    """
    ...
    """
    stmt = select(models.RSE).where(and_(models.RSE.rse == rse, models.RSE.vo == vo))
    if not include_deleted:
        stmt = stmt.where(models.RSE.deleted == false())
...
@transactional_session
def del_rse(rse_id: str, *, session: "Session") -> None:
    """
    ...
    try:
        stmt = select(models.RSE).where(
            and_(models.RSE.id == rse_id, models.RSE.deleted == false())
        )
...
@transactional_session
def restore_rse(rse_id: str, *, session: "Session") -> None:
    """
    ...
    try:
        stmt = select(models.RSE).where(and_(models.RSE.id == rse_id, models.RSE.deleted == true()))
        db_rse = session.execute(stmt).scalar_one()
    except sqlalchemy.orm.exc.NoResultFound:
        raise exception.RSENotFound("RSE with id '%s' cannot be found" % rse_id)
...
@read_session
def rse_is_empty(rse_id: str, *, session: "Session") -> bool:
    """
...
def _format_get_rse(
    ...
    *,
    ...
    if rse_attributes is not None:
        ...
        stmt = select(models.RSEAttrAssociation).where(
            and_(
                models.RSEAttrAssociation.rse_id == db_rse.id,
...
@read_session
def get_rse(rse_id: str, *, session: "Session") -> dict[str, Any]:
    """
    ...
    try:
        stmt = select(models.RSE).where(
            and_(models.RSE.deleted == false(), models.RSE.id == rse_id)
        )
...
@read_session
def get_rse_id(
    rse: str, vo: str = DEFAULT_VO, include_deleted: bool = True, *, session: "Session"
) -> str:
    """
    ...
    if include_deleted:
        if vo != DEFAULT_VO:
            cache_key = f"rse-id_{rse}@{vo}".replace(" ", ".")
        else:
            cache_key = f"rse-id_{rse}".replace(" ", ".")
        result = REGION.get(cache_key)
        if not isinstance(result, NoValue):
            return result

    try:
        stmt = select(models.RSE.id).where(and_(models.RSE.rse == rse, models.RSE.vo == vo))
        if not include_deleted:
            stmt = stmt.where(models.RSE.deleted == false())
...
def _get_rse_db_column(
    ...
    *,
    ...
    if include_deleted:
        cache_key = f"{cache_prefix}_{rse_id}"
        result = REGION.get(cache_key)
        if not isinstance(result, NoValue):
            return result

    try:
        stmt = select(column).where(models.RSE.id == rse_id)
        if not include_deleted:
            stmt = stmt.where(models.RSE.deleted == false())
        result = session.execute(stmt).scalar_one()
    except sqlalchemy.orm.exc.NoResultFound:
...
@read_session
def get_rse_name(rse_id: str, include_deleted: bool = True, *, session: "Session") -> str:
    """
...
@read_session
def get_rse_vo(rse_id: str, include_deleted: bool = True, *, session: "Session") -> str:
    """
...
@read_session
def list_rses(
    filters: Optional[dict[str, Any]] = None, *, session: "Session"
) -> list[dict[str, Any]]:
    """
    ...
    filters = (
        filters.copy()
    )  # Make a copy, so we can pop() without affecting the object `filters` outside this function

    stmt = select(models.RSE).where(models.RSE.deleted == false())
    if filters:
        if "availability" in filters and (
            "availability_read" in filters
    ...
    stmt = stmt.order_by(models.RSE.rse)
...
def add_rse_attribute(
    rse_id: str, key: str, value: Union[bool, str], *, session: "Session"
) -> bool:
...
@transactional_session
def del_rse_attribute(rse_id: str, key: str, *, session: "Session") -> bool:
    """
...
def list_rse_attributes(
    rse_id: str, use_cache: bool = False, *, session: "Session"
    ...
    stmt = select(models.RSEAttrAssociation).where(models.RSEAttrAssociation.rse_id == rse_id)
...
def _fetch_many_rses_attributes(
    ...
    *,
    ...
    if keys:
        stmt = stmt.where(models.RSEAttrAssociation.key.in_(keys))
...
@read_session
def has_rse_attribute(rse_id: str, key: str, *, session: "Session") -> bool:
    """
    ...
    """
    stmt = select(models.RSEAttrAssociation.value).where(
        and_(
            models.RSEAttrAssociation.rse_id == rse_id,
            models.RSEAttrAssociation.key == key,
        )
...
@read_session
def get_rses_with_attribute(key: str, *, session: "Session") -> list[dict[str, Any]]:
    """
    ...
    stmt = (
        select(models.RSE)
        .where(models.RSE.deleted == false())
        .join(
            models.RSEAttrAssociation,
            and_(
...
def get_rses_with_attribute_value(
    key: str, value: Union[bool, str], vo: str = DEFAULT_VO, *, session: "Session"
) -> list[dict[str, str]]:
    """
    Return all RSEs with a certain attribute.
    ...
    if isinstance(result, NoValue):
        ...
        stmt = (
            ...
            .where(and_(models.RSE.deleted == false(), models.RSE.vo == vo))
            .join(
                models.RSEAttrAssociation,
                and_(
                    models.RSEAttrAssociation.rse_id == models.RSE.id,
        ...
        for row in session.execute(stmt):
            rse_list.append({"rse_id": row.id, "rse_name": row.rse})
...
@overload
def get_rse_attribute(
    rse_id: str, key: Literal["sign_url"], use_cache: bool = True, *, session: "Session"
) -> Optional[SUPPORTED_SIGN_URL_SERVICES_LITERAL]: ...


@overload
def get_rse_attribute(
    rse_id: str, key: Literal["sign_url"], use_cache: bool = True
) -> Optional[SUPPORTED_SIGN_URL_SERVICES_LITERAL]: ...


@overload
def get_rse_attribute(
    rse_id: str, key: "RSE_ATTRS_STR", use_cache: bool = True
) -> Optional[str]: ...


@overload
def get_rse_attribute(
    rse_id: str, key: "RSE_ATTRS_STR", use_cache: bool = True, *, session: "Session"
) -> Optional[str]: ...


@overload
def get_rse_attribute(
    rse_id: str, key: "RSE_ATTRS_BOOL", use_cache: bool = True
) -> Optional[bool]: ...


@overload
def get_rse_attribute(
    rse_id: str, key: "RSE_ATTRS_BOOL", use_cache: bool = True, *, session: "Session"
) -> Optional[bool]: ...


@read_session
def get_rse_attribute(
    rse_id: str, key: str, use_cache: bool = True, *, session: "Session"
...
def get_rse_supported_checksums_from_attributes(
...
def get_rse_usage(
    ...
    *,
    ...
    stmt_rse_usage = select(models.RSEUsage).where(models.RSEUsage.rse_id == rse_id)

    if source:
        stmt_rse_usage = stmt_rse_usage.where(models.RSEUsage.source == source)

    db_usages = session.execute(stmt_rse_usage).scalars()
    return _format_get_rse_usage(
        rse_id=rse_id, db_usages=db_usages, per_account=per_account, session=session
    )
    ...
    *,
    ...
    for db_usage in db_usages:
        ...
        if per_account and db_usage.source == "rucio":
            stmt_account_usage = select(models.AccountUsage).where(
...
@read_session
def get_rse_limits(
    rse_id: str, name: Optional[str] = None, *, session: "Session"
) -> dict[str, int]:
    """
    Get RSE limits.
    ...
    stmt = select(models.RSELimit).where(models.RSELimit.rse_id == rse_id)
    if name:
        stmt = stmt.where(models.RSELimit.name == name)
...
def get_rse_transfer_limits(
    rse_id: str, activity: Optional[str] = None, *, session: "Session"
) -> dict[str, Any]:
    """
    ...
    """
    try:
        stmt = select(models.TransferLimit).join_from(
        ...
        for limit in session.execute(stmt).scalars():
            ...
            limits.setdefault(limit_dict["direction"], {}).setdefault(
                limit_dict["activity"], limit_dict
            )
...
def list_rse_usage_history(
    rse_id: str, source: Optional[str] = None, *, session: "Session"
    ...
    """
    stmt = (
        select(models.RSEUsageHistory)
        .where(models.RSEUsageHistory.rse_id == rse_id)
        .order_by(desc(models.RSEUsageHistory.updated_at))
    )
    if source:
        stmt = stmt.where(models.RSEUsageHistory.source == source)
...
@transactional_session
def add_protocol(
    rse_id: str, parameter: dict[str, Any], *, session: "Session"
) -> models.RSEProtocol:
    """
        ...
        for domain in parameter["domains"]:
            if domain not in utils.rse_supported_protocol_domains():
                raise exception.RSEProtocolDomainNotSupported(
                    f"The protocol domain '{domain}' is not defined in the schema."
                )
            for op in parameter["domains"][domain]:
                if op not in RSE_ALL_SUPPORTED_PROTOCOL_OPERATIONS:
                    raise exception.RSEOperationNotSupported(
                        f"Operation '{op}' not defined in schema."
                    )
                ...
                if (type(priority) is not int or priority < 0) and priority is not None:
                    raise exception.RSEProtocolPriorityError(
                        f"The provided priority ({priority}) for operation '{op}' in domain '{domain}' is not supported."
                    )
    ...
    if parameter["scheme"] == "srm":
        if ("extended_attributes" not in parameter) or (
            "web_service_path" not in parameter["extended_attributes"]
        ):
            raise exception.InvalidObject(
    ...
    try:
        ...
        if (
            ("UNIQUE constraint failed" in error.args[0])
            or ("conflicts with persistent instance" in error.args[0])
            or match(
                ".*IntegrityError.*ORA-00001: unique constraint.*RSE_PROTOCOLS_PK.*violated.*",
                error.args[0],
            )
            ...
            raise exception.Duplicate(
                ...
                % (parameter["scheme"], parameter["port"], rse, parameter["hostname"])
            )
        elif (
            "may not be NULL" in error.args[0]
            or match(
                ".*IntegrityError.*ORA-01400: cannot insert NULL into.*RSE_PROTOCOLS.*IMPL.*",
                error.args[0],
            )
            ...
            or match(
                ".*NotNullViolation.*null value in column.*violates not-null constraint.*",
                error.args[0],
            )
...
def get_rse_protocols(
    rse_id: str, schemes: Optional[list[str]] = None, *, session: "Session"
    ...
    stmt = select(models.RSEProtocol).where(*terms)
...
def _format_get_rse_protocols(
    ...
    *,
    ...
    if rse_attributes:
        ...
        lfn2pfn_algorithm = get_rse_attribute(
            _rse["id"], RseAttr.LFN2PFN_ALGORITHM, session=session
        )
    ...
    info = {
        "availability_delete": _rse["availability_delete"],
    ...
    info["protocols"] = sorted(
        info["protocols"], key=lambda p: (p["hostname"], p["scheme"], p["port"])
    )
    return info
...
def update_protocols(
    ...
    *,
    ...
    terms = [
        models.RSEProtocol.rse_id == rse_id,
    ...
    try:
        ...
        if (
            "UNIQUE".lower() in error.args[0].lower() or "Duplicate" in error.args[0]
        ):  # Covers SQLite, Oracle and MySQL error
            raise exception.Duplicate(
                "Protocol '%s' on port %s already registered for  '%s' with hostname '%s'."
...
def update_rse(rse_id: str, parameters: dict[str, Any], *, session: "Session"):
    ...
    for key in parameters:
        if key not in MUTABLE_RSE_PROPERTIES:
            raise exception.InputValidationError(
                f"The key '{key}' does not exist for RSE properties."
            )

    try:
        stmt = select(models.RSE).where(models.RSE.id == rse_id)
        db_rse = session.execute(stmt).scalar_one()
    except sqlalchemy.orm.exc.NoResultFound:
        raise exception.RSENotFound("RSE with ID '%s' cannot be found" % rse_id)
    ...
    for key in parameters:
        if (
            key == "name" and parameters["name"] != old_rse_name
        ):  # Needed due to wrongly setting name in pre1.22.7 clients
...
@read_session
def export_rse(rse_id: str, *, session: "Session") -> dict[str, Any]:
    """
...
@transactional_session
def add_qos_policy(rse_id: str, qos_policy: str, *, session: "Session") -> bool:
    """
    ...
    try:
        ...
        new_qos_policy.update({"rse_id": rse_id, "qos_policy": qos_policy})
...
@transactional_session
def delete_qos_policy(rse_id: str, qos_policy: str, *, session: "Session") -> bool:
    """
    ...
    try:
        stmt = delete(models.RSEQoSAssociation).where(
            and_(
...
@read_session
def list_qos_policies(rse_id: str, *, session: "Session") -> list[str]:
    """
    ...
    qos_policies = []
    try:
        stmt = select(models.RSEQoSAssociation.qos_policy).where(
            models.RSEQoSAssociation.rse_id == rse_id
        )
...
@transactional_session
def fill_rse_expired(rse_id: str, *, session: "Session") -> None:
    """
    ...
    stmt = (
        ...
        .where(
            and_(
                ...
                models.RSEFileAssociation.state.in_(
                    (ReplicaState.AVAILABLE, ReplicaState.UNAVAILABLE, ReplicaState.BAD)
                ),
            )
        )
    )
    ...
    models.RSEUsage(rse_id=rse_id, used=sum_bytes, files=sum_files, source="expired").save(
        session=session
    )
...
def determine_audience_for_rse(rse_id: str) -> str:
    ...
    from rucio.core.oidc import ADMIN_ISSUER_ID, get_capabilities
    # deferred: avoids
...
def determine_scope_for_rse(
    ...
    from rucio.core.oidc import ADMIN_ISSUER_ID, get_capabilities  # deferred, see above
