...
from ...models.privileged_operation_request import PrivilegedOperationRequest
...
def _get_kwargs(
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/v1/decisions/privileged-operations",
    }
...
def sync_detailed(
    *,
    ...
    """ May the subject perform an operation reserved for privileged subjects?
     ...
     For every consumer operation without an endpoint of its own (e.g.
    update_replicas_states, add_account, add_scope, approve_rule). The policy for all of them is:
    privileged subjects
    only. An operation that has its own endpoint is rejected with 400, so each
    operation has exactly one route.
