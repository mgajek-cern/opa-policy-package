...
EXCHANGE_GRANT_TYPE = config_get(
    "oidc",
    "exchange_grant_type",
    False,
...
def _idpsecrets() -> dict:
    ...
    if _IDPSECRETS_CACHE is None:
        try:
            ...
            logging.warning("Failed to parse idpsecrets file: %s", IDPSECRETS, exc_info=True)
            _IDPSECRETS_CACHE = {}
...
def request_token(
    ...
    If oidc.token_strategy == 'exchange' and an account is given, obtain the
    ...
    if token_strategy == "exchange" and account is not None:
        try:
            ...
            acc = account if isinstance(account, InternalAccount) else InternalAccount(account)
            result = get_token_for_account_operation(
                acc, req_audience=audience, req_scope=scope, admin=False
            )
            ...
            logging.warning("Token exchange failed for account %s", account, exc_info=True)
        return None
...
def __load_oidc_configuration() -> bool:
    ...
    try:
        ...
        logging.error('Failed to parse configuration file "%s"', IDPSECRETS, exc_info=True)
        return False
    try:
...
def __get_init_oidc_client(
    token_object: models.Token = None, token_type: str = None, **kwargs
) -> dict[Any, Any]:
    """
    ...
    try:
            ...
            if "issuer_id" in kwargs:
                ...
                client_secret = next(
                    (
                        secrets[i]
                        for i in secrets
                        if "issuer" in secrets[i] and kwargs.get("issuer") in secrets[i]["issuer"]
                    ),
                    None,
                )
        ...
        elif config_get_bool("oidc", "supports_audience", raise_exception=False, default=True):
        ...
        if token_object:
            ...
            if token_type and token:
                ...
                oidc_client.grant[auth_args["state"]].grant_expiration_time = (
                    time_util.utc_time_sans_frac() + 300
                )
            ...
            if client_secret is None:
                raise CannotAuthenticate(
                    "Could not resolve Rucio OIDC Client configuration for the given issuer."
                )
            redirect_url = kwargs.get("redirect_uri")
            if not redirect_url:
            ...
            if not redirect_url:
                raise CannotAuthenticate(
                    "Could not pick any redirect URL(s) from the ones defined "
        ...
        if kwargs.get("first_init", False):
            ...
            if isinstance(resource_for_url, list):
                ...
                auth_url = auth_url + sep + urlencode({"resource": resource_for_url}, doseq=True)
...
def get_auth_oidc(account: str, *, session: "Session", **kwargs) -> str:
    ...
    if not all_oidc_req_claims_present(
        auth_scope, audience, EXPECTED_OIDC_SCOPE, EXPECTED_OIDC_AUDIENCE
    ):
        raise CannotAuthenticate(
            "Requirements of scope and audience do not satisfy minimal requirements of the Rucio server."
        )
    ...
    try:
        ...
        oidc_dict = __get_init_oidc_client(
            issuer_id=issuer_id,
        ...
        oauth_session_params = models.OAuthRequest(
            account=account,
            state=state,
            nonce=nonce,
        ...
        if not auto:
            ...
            auth_url = build_url(
                "https://" + auth_server.netloc,
                path="{}auth/oidc_redirect".format(auth_server.path.split("auth/")[0].lstrip("/")),
                params=access_msg,
            )
...
@transactional_session
def get_token_oidc(
    auth_query_string: str, ip: Optional[str] = None, *, session: "Session"
    ...
    try:
        ...
        query = select(models.OAuthRequest).where(models.OAuthRequest.state == state)
        oauth_req_params = session.execute(query).scalar()
        if oauth_req_params is None:
            raise CannotAuthenticate(
                "User related Rucio OIDC session could not keep "
        ...
        oidc_tokens = oidc_client.do_access_token_request(
            state=state,
        ...
        if oidc_tokens["id_token"]["nonce"] != nonce:
            raise CannotAuthenticate(
                "ID token could not be associated with the Rucio OIDC Client"
        ...
        jwt_row_dict["identity"] = oidc_identity_string(
            oidc_tokens["id_token"]["sub"], oidc_tokens["id_token"]["iss"]
        )
        jwt_row_dict["account"] = oauth_req_params.account

        if jwt_row_dict["account"].external == "webui":
            try:
                jwt_row_dict["account"] = get_default_account(
                    jwt_row_dict["identity"], IdentityType.OIDC, True, session=session
                )
            except Exception:
                return {"webhome": None, "token": None}

        # check if given account has the identity registered
        if not exist_identity_account(
            jwt_row_dict["identity"],
            IdentityType.OIDC,
            jwt_row_dict["account"],
            session=session,
        ):
            raise CannotAuthenticate(
                "OIDC identity '%s' of the '%s' account is unknown to Rucio."
        ...
        if "refresh_token" in oidc_tokens:
            ...
            try:
                ...
                extra_dict["refresh_expired_at"] = datetime.utcnow() + timedelta(
                    hours=REFRESH_LIFETIME_H
                )

        new_token = __save_validated_token(
        ...
        if oauth_req_params.access_msg:
            ...
            if "http" not in oauth_req_params.access_msg:
                if "_polling" not in oauth_req_params.access_msg:
                    fetchcode = rndstr(50)
                    query = (
                        update(models.OAuthRequest)
                        .where(models.OAuthRequest.state == state)
                        .values(
                            {
...
@transactional_session
def __get_admin_token_oidc(
    account: "InternalAccount", req_scope, req_audience, issuer, *, session: "Session"
):
    """
    ...
    try:
        ...
        if "access_token" in oidc_tokens:
            validate_dict = __get_rucio_jwt_dict(
                oidc_tokens["access_token"], account=account, session=session
            )
            if validate_dict:
...
def __get_admin_account_for_issuer(*, session: "Session"):
    ...
    for issuer in OIDC_ADMIN_CLIENTS:
        ...
        query = select(models.IdentityAccountAssociation.account).where(
...
@transactional_session
def get_token_for_account_operation(
    account: str,
    req_audience: str = None,
    req_scope: str = None,
    admin: bool = False,
    *,
    session: "Session",
    ...
    try:
        ...
        query = select(models.IdentityAccountAssociation.identity).where(
            models.IdentityAccountAssociation.identity_type == IdentityType.OIDC,
        ...
        if get_token_for_adminacc:
            ...
            query = select(models.Token).where(
                models.Token.account == account,
                models.Token.expired_at > datetime.utcnow(),
            )
            ...
            for admin_token in admin_account_tokens:
                if (
                    hasattr(admin_token, "audience")
                    and hasattr(admin_token, "oidc_scope")
                    and all_oidc_req_claims_present(
                        admin_token.oidc_scope,
                        admin_token.audience,
                        req_scope,
                        req_audience,
                    )
                ):
            ...
            new_admin_token = __get_admin_token_oidc(
                account, req_scope, req_audience, admin_issuer, session=session
            )
        ...
        if admin and not get_token_for_adminacc:
            ...
            for account_token in account_tokens:
                if all_oidc_req_claims_present(
                    account_token.oidc_scope,
                    account_token.audience,
                    EXPECTED_OIDC_SCOPE,
            ...
            new_admin_token = __get_admin_token_oidc(
                admin_account, req_scope, req_audience, admin_issuer, session=session
            )
            return new_admin_token
        ...
        if not admin and not get_token_for_adminacc:
            ...
            for token in account_tokens:
                if hasattr(token, "audience") and hasattr(token, "oidc_scope"):
                    if all_oidc_req_claims_present(
                        token.oidc_scope, token.audience, req_scope, req_audience
                    ):
            ...
            exchanged_token = __exchange_token_oidc(
                subject_token,
                scope=req_scope,
...
def __exchange_token_oidc(subject_token_object: models.Token, *, session: "Session", **kwargs):
    ...
    if (
        "offline_access" in str(subject_token_object.oidc_scope)
        and not subject_token_object.refresh_token
    ):
        jwt_row_dict["authz_scope"] += " offline_access"
    # Teapot's proxy authenticates via Keycloak's /userinfo endpoint, which
    # rejects tokens lacking the 'openid' scope ("insufficient_scope: Missing
    # openid scope"). Ensure exchanged storage tokens carry openid so they are
    # accepted by OIDC userinfo-based validators (StoRM/Teapot), not just by
    # offline SciTokens validators (XRootD).
    if "openid" not in str(jwt_row_dict["authz_scope"]).split():
        jwt_row_dict["authz_scope"] = ("openid " + jwt_row_dict["authz_scope"]).strip()
    if not grant_type:
        grant_type = EXCHANGE_GRANT_TYPE
    try:
        oidc_dict = __get_init_oidc_client(
            token_object=subject_token_object, token_type="subject_token"
        )  # noqa: S106
...
def __change_refresh_state(token: str, refresh: bool = False, *, session: "Session"):
    ...
    """
    try:
        query = update(models.Token).where(models.Token.token == token)
        if refresh:
            # update refresh column for a token to True
            query = query.values({models.Token.refresh: True})
        else:
            query = query.values(
                {
...
def refresh_cli_auth_token(
    token_string: str, account: str, *, session: "Session"
    ...
    if account_token.refresh:
        ...
        if account_token.updated_at > datetime_min_ago:
            epoch_exp = int(
                floor((account_token.expired_at - datetime(1970, 1, 1)).total_seconds())
            )
    ...
    *,
    ...
    try:
        ...
        if session.bind.dialect.name == "oracle":
            query = (
                select(models.Token)
                .where(models.Token.token.in_(query.with_only_columns(models.Token.token)))
                .with_for_update(skip_locked=True)
            )
        else:
...
def __refresh_token_oidc(token_object: models.Token, *, session: "Session"):
    ...
    try:
        ...
        if datetime.utcnow() - extra_dict["refresh_start"] > timedelta(
            hours=extra_dict["refresh_lifetime"]
        ):
        ...
        if "refresh_token" in oidc_tokens and "access_token" in oidc_tokens:
            ...
            # get access token expiry timestamp
            jwt_row_dict["lifetime"] = datetime.utcnow() + timedelta(
                seconds=oidc_tokens["expires_in"]
            )
            extra_dict["refresh"] = True
            extra_dict["refresh_token"] = oidc_tokens["refresh_token"]
            try:
            ...
            raise CannotAuthorize(
                "OIDC identity '%s' of the '%s' account is did not "
                % (token_object.identity, token_object.account)
...
@transactional_session
def delete_expired_oauthrequests(
    total_workers: int, worker_number: int, limit: int = 1000, *, session: "Session"
):
    """
    ...
    try:
        ...
        query = (
            select(models.OAuthRequest.state)
            .where(models.OAuthRequest.expired_at < datetime.utcnow())
        ...
        if session.bind.dialect.name == "oracle":
            query = (
                select(models.OAuthRequest.state)
                .where(models.OAuthRequest.state.in_(query))
                .with_for_update(skip_locked=True)
            )
        else:
            query = query.with_for_update(skip_locked=True)
        ...
        for states in session.execute(query).scalars().partitions(10):
            query = delete(models.OAuthRequest).where(models.OAuthRequest.state.in_(states))
...
def __get_rucio_jwt_dict(jwt: str, account=None, *, session: "Session"):
    ...
    try:
        ...
        if not account:
            ...
            if not exist_identity_account(
                identity_string, IdentityType.OIDC, account, session=session
            ):
...
def __save_validated_token(token, valid_dict, extra_dict=None, *, session: "Session"):
    ...
    try:
        ...
        new_token = models.Token(
            account=valid_dict.get("account", None),
            token=token,
    ...
    try:
        ...
        if token_dict["authz_scope"] and token_dict["audience"]:
            if all_oidc_req_claims_present(
                ...
                logging.debug(
                    "Token audience [%s] or scope [%s] verification failed.",
                    token_dict["audience"],
                    token_dict["authz_scope"],
                )
                raise CannotAuthenticate(traceback.format_exc())
...
@transactional_session
def save_subject_token(token, account, identity, scope, audience, lifetime, *, session: "Session"):
    """Persist a seeded OIDC subject token into the tokens table for `account`."""
    valid_dict = {
        "account": account,
        "identity": identity,
        "lifetime": lifetime,
    ...
    return __save_validated_token(token, valid_dict, extra_dict={}, session=session)
